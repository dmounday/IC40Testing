...
SerialIO::SerialIO(pt::ptree const &node) : ioc_{1}, port_{ioc_, node.get<std::string>("name")}, timer_{ioc_} {
  ...
  port_.set_option(
      asio::serial_port_base::stop_bits(sbits == 1 ? asio::serial_port_base::stop_bits::one : asio::serial_port_base::stop_bits::two));
